...
export function collided(thing1: PhysicsComponent, thing2:PhysicsComponent, collisionInfo: CollisionInfo) : boolean {
...
function rectCollision(rect:RigidRect, thing:PhysicsComponent, collisionInfo:CollisionInfo) :boolean{
    ...
    function circCollision(circ:RigidCircle, thing:PhysicsComponent, collisionInfo: CollisionInfo) : boolean {
        if (thing instanceof RigidRect) {
...
function rectRectCollision(first: RigidRect, second: RigidRect, collisionInfo:CollisionInfo) {
...
function rectCircCollision(rect:RigidRect, circle:RigidCircle, collisionInfo:CollisionInfo) {
...
function circRectCollision(circle:RigidCircle, rect:RigidRect, collisionInfo:CollisionInfo) {
    const isCollided = rectCircCollision(rect, circle, collisionInfo);
    vec2.scale(collisionInfo.normal, collisionInfo.normal, -1);
    return isCollided;
}

function circCircCollision(first:RigidCircle, second:RigidCircle, collisionInfo:CollisionInfo) {
