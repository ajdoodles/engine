...
export default class GameObjectSet{
    ...
    decSelected () {
        const index =
                (this.selected <= 0 ? this.set.length - 1 : this.selected - 1);
        this.selectObjectAt(index);
    };
        ...
        if (this.isAlertingCollisions()) {
            for (let j = 0; j < this.size(); j++) {
                ...
                for (let k = j+1; k < this.size(); k++) {
                    ...
                    if (theseBounds !== undefined && thoseBounds !== undefined && theseBounds.collided(thoseBounds)) {
