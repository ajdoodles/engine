...
const getCharInfo = function (fontName: string, aChar: number) {
  let returnInfo = null;
  const fontInfo = resourceMap.retrieveAsset(fontName) as Document & {
    fontImage: string;
  };
  const commonPath = "font/common";
  const commonInfo = fontInfo.evaluate(
    commonPath,
    fontInfo,
    null,
  ...
  const charInfo = fontInfo.evaluate(
    charPath,
    fontInfo,
    null,
    XPathResult.ANY_TYPE,
  ...
  const charWidth = Number.parseInt(
    charElem.getAttribute("xadvance") as string
  );
  returnInfo.mCharWidth =
    Number.parseInt(charElem.getAttribute("width") as string) / charWidth;
  returnInfo.mCharHeight =
    Number.parseInt(charElem.getAttribute("height") as string) / charHeight;
  returnInfo.mCharWidthOffset =
    Number.parseInt(charElem.getAttribute("xoffset") as string) / charWidth;
  returnInfo.mCharHeightOffset =
    Number.parseInt(charElem.getAttribute("yoffset") as string) / charHeight;
