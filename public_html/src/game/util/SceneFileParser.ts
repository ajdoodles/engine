...
export default class SceneFileParser {
    ...
    parseCamera() {
        ...
        const viewport = viewportTmp.map((value: string) => Number.parseInt(value)) as [number, number, number, number];
        const bgColorTmp = (cameraInfo.getAttribute("BgColor") as string).split(" ");
        const bgColor = bgColorTmp.map((value: string) => Number.parseInt(value)) as color;
